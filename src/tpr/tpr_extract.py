...
def crop(origin_path, save_path):
    ...
    resized_image = cv2.resize(crop_img, target_size,
...
def _process_image_files_batch(coder, output_file, filenames, synsets, labels):
    ...
    for filename, synset in zip(filenames, synsets):
        ...
        image_data.append(np.array(image_buffer))
        label_data.append(label)

    datas = {'data': np.array(image_data), 'labels': label_data}
...
class DataConverter(object):
    ...
    def to_cifarformat(self, raw_data_dir, local_scratch_dir,
...
if __name__ == '__main__':
    ...
    train_records, validation_records = data_converter.to_cifarformat(
